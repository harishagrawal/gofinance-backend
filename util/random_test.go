...
func TestRandomEmail(t *testing.T) {
	...
	t.Run("Testing length of generated email", func(t *testing.T) {
		...
		if len(emailParts[0]) != length {
			t.Errorf("Expected length of %d but got %d", length, len(emailParts[0]))
		} else {
	...
	t.Run("Testing negative number input", func(t *testing.T) {
		defer func() {
			if r := recover(); r != "runtime error: index out of range" {
				t.Errorf("Code didn't panic as expected for negative input")
			} else {
