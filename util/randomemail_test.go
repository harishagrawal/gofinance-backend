...
func TestRandomEmail(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			number:     5,  // testData can be modified as per requirement, but keep it more than 0
			errorExpected: false,
		},
		{
			name:       "Generate valid random email for the zero number.",
			number:     0,  
			errorExpected: false, // as zero doesn't break functionality it shouldn't return error
		},
		{
			name:       "Generate valid random email for a very large number",
			number:     99999, // testData can be modified with very large number
			errorExpected: false,
		},
		{
			name:       "Generate valid random email for negative numbers.",
			number:     -3,  // testData can be modified with any negative value
			errorExpected: true, // random length can't be negative so it should ideally return an error
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if (err != nil) != tc.errorExpected {
				t.Errorf("\nTest: %s\nError was expected: %v\nError occured: %v\n", tc.name, tc.errorExpected, err)
				return
			}

			// Assert
			if err == nil {
				randomStringLen := strings.Index(email, "@")
				if tc.number != randomStringLen {
					t.Errorf("\nTest: %s\nExpected random string length: %d\nActual random string length: %d\n",
						tc.name, tc.number, randomStringLen)
				}
			}
		})
	}
