...
func TestRandomEmail(t *testing.T) {
	...
	t.Run("Testing negative number input", func(t *testing.T) {
		email := RandomEmail(-10)
		if len(email)!=11{
			t.Errorf("Email generation with negative input is incorrect")
		} else {
			t.Logf("Email generation with negative input handled correctly")
		}
	})
}
...
func TestRandomString(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:     "Negative input",
			input:    -1,
			expected: 0, // Expect empty string on negative input
		},
	}
